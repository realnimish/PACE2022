...
/**
 * @param OG original graph
 * @return FVS of the original graph using SA
 */
vector<int> get_fvs_sa(Graph* OG) {
    stack<Graph*> st; // to store objects of induced graphs
    vector<int> fvs; // stores the FVS
...
int main() {
    ...
    vector<int> fvs_fast = get_fvs(G);
    vector<int> fvs_sa = get_fvs_sa(G);

    if(fvs_fast.size() < fvs_sa.size()) {
        for(const int& x: fvs_fast)
            cout << x << "\n";
    } else {
        for(const int& x: fvs_sa)
            cout << x << "\n";
    }
