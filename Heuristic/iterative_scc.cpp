...
unordered_set<int> get_redunant(Graph* OG, vector<int>& fvs) {
    ...
    int batch_size = 5000; // process {batch_size} no. of nodes at one go
...
vector<int> improve_fvs(Graph* G, int orig_edges) {
    ...
    if(orig_edges > 113000 && orig_edges <= 284000)
    {
        freq = 50;
        max_iter = 20;
        start_add = 5; //think on this
    }
    else if(orig_edges > 284000 && orig_edges <= 459000)
    {
        freq = 50;
        max_iter = 10;
        start_add = 10;
    }
    else if(orig_edges > 459000)
    {
        freq = 50;
        max_iter = 10;
        start_add = 50;
    ...
    while(start_add && !tle)
    {
        ...
        for(int iter=1; iter<=max_iter && !tle; ++iter)
        {
            ...
            int num_nodes_add = max(1, curr_sz/start_add);
            num_nodes_add = min(num_nodes_add, graph_nodes_sz - curr_sz);
            ...
            {
                int nd = graph_nodes[rand()%graph_nodes_sz];
            ...
            // cerr << "Before curr size: " << curr_fvs.size() << "\n";
            curr_fvs = remove_redundant(DG, curr_fvs, freq);
            // cerr << "After curr size: " << curr_fvs.size() << "\n";
            delete(DG);
        ...
        if(best_fvs.size() < best_add_size)
        {
            ...
            best_add = best_fvs;
            // cerr << "Best FVS: " << best_add_size << "\n";
        }
        

        start_add++;
    }
    // best_add = remove_redundant(G, curr_fvs, freq);
    // cerr << best_add.size() << "\n";
...
int main() {
    ...
    G->pre_compute_density();
    vector<int> best_fvs = improve_fvs(G, m);
    ...
    // cerr << "Ans: " << best_fvs.size() + mfvs.size() << "\t\t" << "Time: " << duration.count() << "\n";
}
